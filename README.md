...
| **Server logs `Application startup complete` and `HTTP server started`, then refuses every connection** | **Intermittent and unexplained.** `ss` shows the port in `LISTEN` owned by the live API-server pid, yet connections from the same WSL session — and from Windows — get `ECONNREFUSED`. Ruled out: plain WSL loopback is healthy (a trivial python server on the same interface serves fine), the port is not in a Windows reserved range, and nothing holds it Windows-side | **No reliable fix known.** An earlier version of this file claimed it was specific to port 8000 and that changing port fixed it — **that was wrong**: port 8000 has both worked and failed, and port 8142 later failed the same way while 8137 worked. Budget for retries; probe readiness rather than trusting the startup log. This has cost eight runs so far |
